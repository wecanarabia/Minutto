<x-layouts.header title="Edit Vacation Type"/>
<x-layouts.app>
    <!-- Body: Body -->
           ...
           <div class="row align-items-center">
               <div class="border-0 mb-4">
                   <div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
                       <h3 class="fw-bold mb-0">Edit Vacation Type</h3>
                       <div class="col-auto d-flex w-sm-100">
                        <a class="btn btn-dark btn-set-task w-sm-100" href="{{ route('company.vacation-types.index') }}"></i>Vacation Types</a>
                        <a class="btn btn-dark btn-set-task w-sm-100 mx-2 " href="{{ route('company.vacation-types.show',$type->id) }}"></i>{{ $type->name }}</a>
                    </div>
                   </div>
               </div>
           </div> <!-- Row end  -->
               ...
               <div class="col-md-12">
                   <div class="card mb-3">
                       <div class="card-header py-3 d-flex justify-content-between bg-transparent border-bottom-0">
                           <h6 class="mb-0 fw-bold ">Edit Vacation Type</h6>
                       </div>
                       <div class="card-body">
                        <form method="post" action="{{ route('company.vacation-types.update',$type->id) }}">
                            @csrf
                            @method('PUT')
