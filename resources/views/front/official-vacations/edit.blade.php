<x-layouts.header title="Edit Official Vacation"/>
<x-layouts.app>
    <!-- Body: Body -->
           ...
           <div class="row align-items-center">
               <div class="border-0 mb-4">
                   <div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
                       <h3 class="fw-bold mb-0">Edit Official Vacation</h3>
                       <div class="col-auto d-flex w-sm-100">
                        <a class="btn btn-dark btn-set-task w-sm-100" href="{{ route('company.official-vacations.index') }}"></i>Official Vacations</a>
                        <a class="btn btn-dark btn-set-task w-sm-100 mx-2 " href="{{ route('company.official-vacations.show',$vacation->id) }}"></i>{{ $vacation->name }}</a>
                    </div>
                   </div>
               </div>
           </div> <!-- Row end  -->
               ...
               <div class="col-md-12">
                   <div class="card mb-3">
                       <div class="card-header py-3 d-flex justify-content-between bg-transparent border-bottom-0">
                           <h6 class="mb-0 fw-bold ">Edit Official Vacation</h6>
                       </div>
                       <div class="card-body">
                        <form method="post" action="{{ route('company.official-vacations.update',$vacation->id) }}">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="id" value="{{ $vacation->id }}">
                                ...
                                <div class="row g-3 align-items-center">
                                   <div class="col-md-6">
                                       ...
                                       <input type="text" class="form-control" id="firstname" name="english_name" value="{{ old('english_name',$vacation->getTranslation('name', 'en')) }}" required>
                                       @error('english_name')
                                   ...
                                   <div class="col-md-6">
                                       ...
                                       <input type="text" class="form-control" name="arabic_name" value="{{ old('arabic_name',$vacation->getTranslation('name', 'ar')) }}" id="lastname" required>
                                       @error('arabic_name')
                                       <div class="text-danger">{{ $message }}</div>
                                       @enderror
                                   </div>
